...
class LoraTraner: 
    ...
    @staticmethod
    def get_config_2(sdxl: str, user_name: str, sample_every_n_type_value) -> dict:
        config = {
            "model_arguments": {
                "v2": cfg.v2,
                "v_parameterization": cfg.v_parameterization if cfg.v2 and cfg.v_parameterization else False,
                "pretrained_model_name_or_path": cfg.pretrained_model_name_or_path if sdxl == "0" else None,
                "vae": cfg.vae,
            },
    ...
    def run(self, train_data_dir, user_name,  sdxl, is_male):
        try: 
            ...
            self.write_config(
                self.transfer_json_to_toml(
                    self.replace_empty_with_none(
                        self.get_config_2(sdxl, user_name, sample_every_n_type_value)
                    )
                ), 
